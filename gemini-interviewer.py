...
class InterviewBot:
    def __init__(self, api_key: str):
        """Initialize the InterviewBot with Gemini API key."""
        ...
        # Initial system prompt to guide the Gemini model
        self.system_prompt = """
    ...
    def generate_next_question(self, user_response: str = None) -> str:
        ...
        if self.question_count == 1:
            prompt = self.system_prompt + f"\nGenerate Question {self.question_count}:"
            self.follow_up_count = 0  # Initialize follow-up count
        else:
            # Add the user response to conversation history
            self.conversation_history.append({"role": "user", "parts": [user_response]})
            ...
            # Create a context-aware prompt for the next question
            ...
            for message in self.conversation_history:
                if message["role"] == "model":
                    prompt += f"AI: {message['parts'][0]}\n"
                else:
                    prompt += f"User: {message['parts'][0]}\n"
            ...
            if self.follow_up_count >= self.max_follow_ups:
                ...
                prompt += f"For Question {self.question_count}, introduce a completely new topic unrelated to previous questions:"
                self.follow_up_count = 0  # Reset follow-up count for new topic
            else:
                prompt += f"\nThis is follow-up #{self.follow_up_count} on the current topic. "
                prompt += f"Generate Question {self.question_count} based on the conversation:"
        ...
        self.conversation_history.append({"role": "model", "parts": [question_text]})
...
def run_interview():
    ...
    # Estimate cost (based on approximate Gemini Pro pricing)
    input_cost = token_usage['input_tokens'] * 0.00000125  # $0.00125 per 1K input tokens
    output_cost = token_usage['output_tokens'] * 0.00000375  # $0.00375 per 1K output tokens
...
if __name__ == "__main__":
    ...
    try:
        ...
        os.system("pip install google-generativeai tiktoken")
